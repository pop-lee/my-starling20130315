package com.sanrenxing.tb.screens
{
	...
	public class ProductListScreen extends Screen
	{
		 ...
		 * 该类商品的所有商品列表
		 */
		private var _protuctsArr:Vector.<ProductImage>=new Vector.<ProductImage>();
		/**
		 * 当前聚焦的产品在所有产品中的索引位置
		 */
...
//		private var _isToDetail:Boolean=false;
		...
		private var _curTouchProduct:ProductImage;
...
//				}
		...
		private function initUI():void
		{
			...
			for(var i:int=0;i<showProductLength;i++) { //同屏显示个数
				...
				var product:ProductImage=new ProductImage(data.productListVO[i]);
			...
			var l:int=0;
			for(var j:int=_focusProductIndex;j<=rightIndex;j+=l) {
				var productMoveTween:Tween;
				if(j==_focusProductIndex) {
					if(_protuctsArr[j].parent==null) {
						addProductToStage(j);
					}
					productMoveTween = new Tween(_protuctsArr[j],1,Transitions.EASE_OUT_BACK);
					productMoveTween.onComplete = productMoveTweenCompleteHandler;
				} else {
					addProductToStage(j);
					productMoveTween = new Tween(_protuctsArr[j],0.7,Transitions.EASE_IN_OUT);
				}
				productMoveTween.animate("x",calculateExpectX(j));
				Starling.juggler.add(productMoveTween);
				
				if(j==leftIndex) {
					j=_focusProductIndex;
					l=1;
				} else {
					if(j==_focusProductIndex) {
						l=-1;
...
//		}
		 ...
		 */
		protected function addProductToStage(index:int):ProductImage
		{
...
//			}
		 ...
		 */
		protected function removeProductFromStage(index:int):ProductImage
		{
			if(index<0||index>_protuctsArr.length-1) return null; //如果超出范围，则直接return
			...
			return this._container.removeChild(_protuctsArr[index]) as ProductImage;
		}
		...
		private function touchProductHandler(event:TouchEvent):void
		{
			var touch:Touch = event.getTouch(stage);
			if(touch.phase == "began") {
				_curTouchProduct = event.currentTarget as ProductImage;
			}
		}
